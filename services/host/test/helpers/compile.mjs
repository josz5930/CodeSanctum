...
const outDir = path.join(repoRoot, "node_modules", ".cache", "host-test-dist");
...
async function isFresh() {
  try {
    ...
    const newestSource = Math.max(
      ...
      await newestMtime(path.join(repoRoot, "packages", "evidence-store", "src")),
      await newestMtime(path.join(repoRoot, "packages", "identity-store", "src")),
      await newestMtime(path.join(repoRoot, "packages", "protocol-ts", "src")),
      await newestMtime(path.join(repoRoot, "packages", "signing", "src")),
...
export async function compileWorkspace() {
  ...
  execFileSync(process.execPath, [
    ...
    outDir,
    "--tsBuildInfoFile",
    path.join(outDir, "host-test.tsbuildinfo")
  ], {
    cwd: workspacePath,
    stdio: "pipe"
  });
...
export async function importCompiled(relativePath) {
  ...
  return import(pathToFileURL(path.join(dir, "services", "host", relativePath)).href);
}
